...
void sysCalls_enableLed(bool turnOn, int led) {
    volatile SysCallArgs_t args = { SYSCALL_LED, turnOn, led };
    makeSysCall(args);
}

int sysCalls_openFile(const char* fileName) {
    volatile SysCallArgs_t args = { SYSCALL_FILE_OPEN, (int) fileName };
    return makeSysCall(args);
}

int sysCalls_readFile(int fileDescriptor, uint8_t* buffer, unsigned int bufferSize) {
    volatile SysCallArgs_t args = { SYSCALL_FILE_READ, fileDescriptor, (int) buffer, bufferSize };
    return makeSysCall(args);
}

void sysCalls_writeFile(int fileDescriptor, const uint8_t* buffer, unsigned int bufferSize) {
    volatile SysCallArgs_t args = { SYSCALL_FILE_WRITE, fileDescriptor, (int) buffer, bufferSize };
    makeSysCall(args);
}

void sysCalls_closeFile(int fileDescriptor) {
    volatile SysCallArgs_t args = { SYSCALL_FILE_CLOSE, fileDescriptor };
    makeSysCall(args);
}

const char* sysCalls_readDirectory(const char* directoryName) {
    volatile SysCallArgs_t args = { SYSCALL_READDIR, (int) directoryName };
